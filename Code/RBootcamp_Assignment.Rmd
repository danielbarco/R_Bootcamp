...
# to specify python 3.7. unfortunately there is no way to solve this in a generic approach that works on all machines.
set.seed(69)
```
...
ts$sin_yearday = def_sin(ts$day_count, 365)
ts$cos_yearday = def_cos(ts$day_count, 365)

ts$sin_weekday <- as.numeric(ts$sin_weekday)
ts$cos_weekday <- as.numeric(ts$cos_weekday)
ts$sin_yearday <- as.numeric(ts$sin_yearday)
ts$cos_yearday <- as.numeric(ts$cos_yearday)
```
...
ts$isferiengr = factor(ts$isferiengr, levels = c(unique(ts$isferiengr)), labels=c(seq(0,1)))
ts$isferiensg = factor(ts$isferiensg, levels = c(unique(ts$isferiensg)), labels=c(seq(0,1)))
ts$isferienzh = factor(ts$isferienzh, levels = c(unique(ts$isferienzh)), labels=c(seq(0,1)))
```
...
### Let us check the coefficients
```{r}
str(summary(lm.fit), max.level = 1)
str(lm.fit, max.level = 1
    )
class(lm.fit)
summary(lm.fit)
```
...
```{r}
rf.dd=randomForest(schalter~.,data=dd_train,mtry=8,importance=TRUE,ntree=25)
yhat.rf = predict(rf.dd,newdata=dd_validate)
test.err= mean( (dd_validate$schalter - yhat.rf)^2)
print(test.err)
#importance(rf.dd)
varImpPlot(rf.dd, n.var = 6, sort = TRUE, main = "Random Forest 8 most important variables" )
```

```{r}
rf.fit=randomForest(schalter~. ,data=dd_train,mtry=8,importance=TRUE)
rf.fit
...
rf.mse <- with(dd_validate, mean( (schalter - rf.pred)^2))
rf.mse
...
The python output for this model is not as easily interpretable as in the R script. The R script follows a convenient layout wheras in pyhton this information must be extracted from the model.

Unfortunately not all pyhton functionalities are possible in R, which makes preprocessing and especially preprocessing impossible.

 ![Alt Text](https://media.giphy.com/media/3o72F7JTbNletrGzvO/giphy.gif) 
  ![Alt Text](https://media.giphy.com/media/10tuFEeuACAnuw/giphy.gif) 
